...
type PageRepository interface {
	...
	CreateShareLink(ctx context.Context, pageID, userID uuid.UUID, permission string, expiresAt time.Time) (*models.ShareLink, error)
	ListPages(ctx context.Context, userID uuid.UUID, sectionID *uuid.UUID, page, size int, sortBy, order string) ([]models.Page, int64, error)
}
...
func (r *pageRepository) PermanentDelete(ctx context.Context, id uuid.UUID) error {
	return r.db.WithContext(ctx).Unscoped().Delete(&models.Page{}, "id = ?", id).Error
}

func (r *pageRepository) CreateShareLink(ctx context.Context, pageID, userID uuid.UUID, permission string, expiresAt time.Time) (*models.ShareLink, error) {
...
func (r *pageRepository) ListPages(ctx context.Context, userID uuid.UUID, sectionID *uuid.UUID, page, size int, sortBy, order string) ([]models.Page, int64, error) {
	var pages []models.Page
	var total int64
	...
	if err := query.Offset(offset).Limit(size).Order(sortOrder).Find(&pages).Error; err != nil {
		return nil, 0, err
	}
...
type PageRepository interface {
	...
	CreateShareLink(ctx context.Context, pageID, userID uuid.UUID, permission string, expiresAt time.Time) (*models.ShareLink, error)
	ListPages(ctx context.Context, userID uuid.UUID, sectionID *uuid.UUID, page, size int, sortBy, order string) ([]models.Page, int64, error)
	PurgeOldTrash(ctx context.Context, days int) error
...
func (r *pageRepository) CreateShareLink(
	ctx context.Context, 
	pageID, userID uuid.UUID, 
	permission string, 
	expiresAt time.Time,
...
func (r *pageRepository) ListPages(
	ctx context.Context, 
	userID uuid.UUID, 
	sectionID *uuid.UUID, 
	page, size int, 
...
func (r *pageRepository) PurgeOldTrash(ctx context.Context, days int) error {
	threshold := time.Now().AddDate(0, 0, -days)
	return r.db.WithContext(ctx).
		Unscoped().
		Where("is_deleted = ? AND deleted_at < ?", true, threshold).
		Delete(&models.Page{}).
		Error
}